...
`resolveProjectDir`（`git rev-parse --git-common-dir` + `dirname`）は CLI 側で実行する。`pnpm dev` 時はアプリ側で同期版の `resolveProjectDir`（`Bun.spawnSync`）を使用する。
...
- open メッセージの dir は CLI 側で `resolveProjectDir` によりプロジェクトディレクトリに解決される
