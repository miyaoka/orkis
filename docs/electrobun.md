...
パス解決はすべて desktop 側で行う。CLI は受け取ったパスをそのまま送信する。desktop 側で `resolveProjectDir`（`--git-common-dir` + `dirname`）でプロジェクト判定、`resolveWorktreeRoot`（`--show-toplevel`）で worktree ルートを解決する。
...
- open メッセージの dir は CLI からの生パス。desktop 側で `resolveProjectDir` / `resolveWorktreeRoot` により解決される
