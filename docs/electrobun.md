...
CLI cold start 時に、アプリ起動前に `/tmp/orkis-stable-launch/{uuid}.json` へ `{ dir, file }` を書く。アプリは起動時に `readdirSync` + `readFileSync` で同期的に読み取り、処理済みファイルは `.claimed` にリネームする。TTL（30秒）超過のファイルは起動時に掃除する。
